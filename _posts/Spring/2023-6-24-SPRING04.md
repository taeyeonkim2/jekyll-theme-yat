...
BoardController 클래스 내부의 @RequestMapping("/boardInsert.do")어노테이션이 걸린

boardInsert() 메서드를 호출할 거고,

(2)DispatcherServlet에 "/memberList.do" 요청이 오면,

HandlerMapping은 이미 읽어 들인 것을 바탕으로 

MemberController 클래스 내부의 @RequestMapping("/memberList.do")어노테이션이 걸린

memberList() 메서드를 호출하는 거지
