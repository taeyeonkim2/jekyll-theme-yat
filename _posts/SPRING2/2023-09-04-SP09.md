...
2.endPage(화면에서 보여지는 마지막 페이지 번호) : 전체 게시물의 수를 한 페이지에서 보여줄 게시물의 수로 나누자

- ex) 현재 페이지 : 17, 화면에서 보여줄 페이지의 개수 : 5 일 때, 17/5 =3.4 

- 일반화 해보면, Math.ceil(totalCount/(double)cri.perPageNum) 
